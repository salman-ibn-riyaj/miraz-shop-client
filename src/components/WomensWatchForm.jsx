...
export default function WomensWatchForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const productData = {
                ...
                strapColor: form.strapColor.value,
            ...
            const res = await fetch(`${baseUrl}/api/products/womens-watch`, {
                method: "POST",
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <h3 className="text-lg font-bold">Women's Watch Details</h3>
            {success && (
                <div className="p-3 bg-success/10 text-success rounded-lg flex items-center gap-2">
                    <FiCheckCircle /> Added to Women's Watch Collection!
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="text" name="dialColor" placeholder="Dial Color" className="p-3 border rounded-xl bg-background" />
                <input type="text" name="strapColor" placeholder="Strap Color / Design" className="p-3 border rounded-xl bg-background" />
                <input type="text" name="warranty" placeholder="Warranty (e.g. 1 Year)" className="p-3 border rounded-xl bg-background" />
            ...
            >
                {loading ? "Uploading Image & Saving..." : "Add Women's Watch"}
            </button>
        </form>
    );
}