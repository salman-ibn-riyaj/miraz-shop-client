...
const ProductCard = ({ product }) => {
  const { title, price, imageUrl, brand } = product;
    ...
    <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-slate-300">
      ...
      <div className="mt-4 flex flex-1 flex-col justify-between">
        ...
        <div className="mt-4 flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
          ...
          <button className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-md transition-all duration-200 hover:bg-slate-800 active:scale-95">
            Buy Now
          </button>
        </div>
      </div>
